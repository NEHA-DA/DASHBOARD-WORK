...
---TASK-1 DISPLAY ORDER STATUS PLACED IN 2025---
SELECT
C.CUSTOMER_NAME ,
C.CITY,
C.CUSTOMER_SEGMENT,
...
---TASK-2 customers from Gujarat who belong to the Premium or Enterprise segment. Display customer name, city, and segment.
...
FROM Customers
WHERE state = 'GUJARAT'
AND customer_segment IN ('PREMIUM' ,'ENTERPRICE');




---TASK-3 Display all products whose unit price is between ?5,000 and ?20,000, sorted from highest price to lowest price.

SELECT *
FROM Products
WHERE unit_price BETWEEN 5000 AND 20000 ;

---TASK-4  Find all completed orders where the quantity is greater than 4. Display order ID, customer ID, product ID, quantity, and order date.
...
---TASK-5 Display the five most expensive products. Show product name, category, and unit price.
...
--TASK-6 Find the total number of customers in each customer segment.
...
---TASK-7 Calculate the total quantity of products ordered for each product.
...
---TASK-8  Find the average unit price of products in each category
...
---TASK-9 Find categories whose average product price is greater than ?10,000.
...
---TASK-10  Find customers who have placed at least 3 orders. Display customer ID and order count.
...
---task-11 Display customer name, order ID, product name, quantity, and unit price using an INNER JOIN between Customers, Orders, and Products.
...
---task-12 Create a sales report showing customer name, product name, category, quantity, unit price, and calculated Total Value (quantity × unit price).
...
task-13 Find the total sales value generated by each customer, considering all orders. Display customer name and total sales value.
...
task-14 ---ind the total sales value for each product category. Display category and total sales value---
select
p.category,
sum(o.quantity * p.unit_price) as total_sales_value
from Orders o
...
task-15  ---Find the number of orders handled by each sales employee. Display employee name, department, and order count.
...
GROUP BY
    ...
    task-16 --find sales employee who handle more then 8 orders--
...
GROUP BY
    ...
    task-18 --customer not placed order---
    ...
    task-19 --- customer placedmore then grether then 3 orders--
    ...
    task-21 total completed value more then 150000
...
task-22 --- sales value more then 200000 ---
...
GROUP BY
    e.employee_name,
    e.department
HAVING SUM(o.quantity * p.unit_price) > 200000;



task-23 --- find top 5 customer by complterd ordr value---
SELECT TOP 5
    c.customer_name,
...
task-24-- find the product with high total quantity--
...
GROUP BY
    ...
    task-25   --create a customer payment --
...
INNER JOIN Payments p
    ...
    TASK-26 --Find all completed orders for which the payment status is not Paid.--
    ...
    task-27 --Find customers who have orders but no support tickets.--
    ...
    task-28 --Find customers who have support tickets but no orders.
    ...
    task-29 ---Display all employees and the number of support tickets assigned to them, including employees with zero tickets.
    ...
    task-30 --Find support employees who handled more than 2 tickets.
    ...
    task-31 --- Create a report containing customer name, order ID, product name, sales employee name, department, quantity, and total order value.

    select
    c.customer_name,
    o.order_id,
    p.product_name,
    e.employee_name as sales_employee,
    e.department,
    o.quantity,
    ...
    task-32 --Find the total revenue generated by each sales employee from completed orders only.
    select
    e.employee_id,
    e.employee_name,
    sum(o.quantity * p.unit_price) as total_revenue
    ...
    task-33 --Find sales employees whose completed-order revenue is greater than ₹500,000.

    select
    e.employee_name,
    sum(o.quantity * p.unit_price) as total_revenue
    from Employees e
    join Orders o on e.employee_id = o.sales_employee_id
    ...
    task-34 --- Find the total completed sales value by customer city.
    ...
    task-35 --Find cities whose completed sales value is greater than ₹300,000.
   ...
   select
    ...
    task-36 ---Using FULL OUTER JOIN, produce a comparison between customers with orders and customers represented in support tickets. Show customer ID, customer name, order ID, and ticket ID.
    select
    c.customer_id,
    c.customer_name,
    o.order_id,
    ...
    on c.customer_id = o.customer_id
    full outer join SupportTickets s
    on c.customer_id = s.customer_id;


    task-37 --Using FULL OUTER JOIN, identify customers who exist only in the order activity or only in the support-ticket activity.
    ...
    task-38 --Using FULL OUTER JOIN, identify customers who exist only in the order activity or only in the support-ticket activity.

select distinct o.customer_id , s.customer_id
from Orders o
full outer join SupportTickets s
on o.customer_id = s.customer_id
where o.customer_id is null
...
task-39 ---Use CROSS JOIN to generate every possible combination of customer segment and product category. Display the segment and category.
...
task-40 --Use CROSS JOIN to generate every possible combination of sales employee and product category, then display the combinations sorted by employee name and category.
...
task-41  --Find the number of distinct customers who purchased products from each product category.
...
task-42 -- Find product categories purchased by at least
select
p.category,
count(o.customer_id) as customer_count
from products p
...
task-43--Find customers who purchased at least 3 different products.
...
task-44 --Find the average completed order value for each customer segment.
...
task-45 -- ind customer segments whose average completed order value is greater than ₹50,000.
...
where o.order_status = 'completed'
group by c.customer_segment 
having avg(o.quantity *unit_price)> 50000;


task-46--- Create a business performance report by sales employee showing employee name, department, number of completed orders, total quantity sold, total revenue, and average order value.
...
task-47 -- Find the sales employee with the highest completed-order revenue without using subqueries or window functions.
...
task-48 ---Create a report for each product showing product name, category, current stock, total quantity ordered, completed sales value, and number of distinct customers who ordered it. Products wit
...
GROUP BY
    ...
    task-49 --or each customer segment, compare completed-order revenue for pairs of customers in the same segment. Display the two customer names and their revenues only when the first customer's revenue is greater than the second customer's revenue.

SELECT
    c1.customer_segment,
    c1.customer_name AS customer_1,
    SUM(o1.quantity * p1.unit_price) AS revenue_1,
    c2.customer_name AS customer_2,
    SUM(o2.quantity * p2.unit_price) AS revenue_2
FROM Customers c1
JOIN Orders o1
...
GROUP BY
    ...
    c1.customer_name,
    c2.customer_name
HAVING SUM(o1.quantity * p1.unit_price)
     > SUM(o2.quantity * p2.unit_price);


task-50  --- Build a management-level sales report showing customer segment, number of customers, number of completed orders, total quantity sold, total revenue, and average order value. Include segments even if a metric has no matching records.
